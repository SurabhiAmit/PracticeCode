...
def cellular_automaton(string,pattern_number,n):
    if n<=0 or string=='' or pattern_number< 0:
        return None
    p=[1,2,4,8,16,32,64,128]
    patterns={1:'...',2:'..x',4:'.x.',8:'.xx',16:'x..',32:'x.x',64:'xx.',128:'xxx'}  
    selected=[]
    value={}
    for i in range(len(p)-1,-1,-1):
        if pattern_number >= p[i]:
            selected.append(p[i])
            pattern_number-=p[i] 
    for e in p:
        if e in selected:
            ...
            value[e]='.'
    if len(string)==1:                       # special case for len(string)=1
        result=string
        while n>0:
            result=3*result[0]
            for e in p:
                if result==patterns[e]:
                    result= value[e] 
                    #print result
            n-=1
        return result 
    if len(string)==2:                        # special case for len(string)=2
        previous=string
        while n>0:
            result=''
            for i in range(0,2):
                if i==0:
                    k=previous[1]+previous[0]+previous[1]
                if i==1:    
                    k=previous[0]+previous[1]+previous[0]
                for e in p:    
                    if k==patterns[e]:
                        result+=value[e]
            previous=result
            n-=1
        return result    
    return result_n(string,n,value)        
    
    
def make_next_row(string,value):
    p=[1,2,4,8,16,32,64,128]
    next_row=''
    patterns={1:'...',2:'..x',4:'.x.',8:'.xx',16:'x..',32:'x.x',64:'xx.',128:'xxx'}    
    first=string[len(string)-1]+string[:2]
    for e in p:
        if first==patterns[e]:
            next_row=next_row+value[e]
    i,j=0,0
    while i<=len(string)-3 and j<=len(string):
        j=i+3
        for e in p:
            if string[i:j]==patterns[e]:
                next_row=next_row+value[e]
        i+=1    
    last=string[-2:]+string[0:1]  
    for e in p:
        if last==patterns[e]:
            next_row=next_row+value[e]
    return next_row  
    
def result_n(string,n,value):
    current=string
    while n>0:
        result=make_next_row(current,value)
        #print value,'\n',result
        current=result
        n-=1
    return result    
