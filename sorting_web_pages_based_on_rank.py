# Define a procedure, ordered_search(index,ranks, keyword), that takes the same inputs as lucky_search from Question 5,
...
def ordered_search(index, ranks, keyword):
    ...
    if not urls:
        return None
    i,j=0,0
    while i < (len(urls)-1): #i=0
        j=i+1
        while j < len(urls):
            if ranks[urls[i]]<ranks[urls[j]]:
